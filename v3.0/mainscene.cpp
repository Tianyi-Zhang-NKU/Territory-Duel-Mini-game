...
// 构造函数
{
    ...
    connect(enemy_plane_timer, &QTimer::timeout, this, &MainScene::createenemy_plane);
    enemy_plane_timer->start(ENEMY_PLANE_COOLDOWN);

    // 创建定时器，每隔16秒创建一个新敌方云朵
    cloud_timer = new QTimer(this);
    connect(cloud_timer, &QTimer::timeout, this, &MainScene::createcloud);
...
//深红色方块初始化（出场）
void MainScene::dred_blocktoscene()
{
    for(int i=0;i<DBLOCK_NUM;i++){
        //如果是空闲且存活红色方块 出场
...
//“激活”深红色方块
void MainScene::dred_blockactivate()
{
...
//敌机“碰撞检测”
void MainScene::collisionDetection_enemy_plane()
{
    //遍历所有空闲的深红色方块
    for(int i = 0 ;i <DBLOCK_NUM;i++){
...
//敌机制造
void MainScene::createenemy_plane()
{
